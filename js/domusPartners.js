...
(function() {
    ...
    Domus.Partners = (function() {
        ...
        function bindContactActions(container = document) {
            ...
            container.querySelectorAll('.domus-partner-name-link').forEach(button => {
                if (button.dataset.domusBound === 'true') {
                    return;
                }
                button.dataset.domusBound = 'true';
                button.addEventListener('click', event => {
                    ...
                    Domus.Router.navigate('partnerDetail', [partnerId]);
                });
            });
        }

        function renderInline(partners, options = {}) {
            const includeTypeColumn = options.includeTypeColumn !== false;
            const includeEmailColumn = options.includeEmailColumn !== false;
            const headers = [t('domus', 'Name')];
            if (includeTypeColumn) {
                headers.push(t('domus', 'Type'));
            }
            if (includeEmailColumn) {
                headers.push(t('domus', 'Email'));
            }
            const rows = (partners || []).map(partner => {
                const cells = [renderPartnerContact(partner, { linkNameToDetail: options.linkNameToDetail === true })];
                if (includeTypeColumn) {
                    cells.push(Domus.Utils.escapeHtml(getPartnerTypeLabel(partner.partnerType)));
                }
                if (includeEmailColumn) {
                    cells.push(Domus.Utils.escapeHtml(partner.email || ''));
                }
                return cells;
            });
            ...
            Domus.Api.get('/partners/' + id)
                .then(partner => {
                    ...
                    const documentsHeader = Domus.UI.buildSectionHeader(t('domus', 'Documents'), documentActionsEnabled ? {
                        id: 'domus-partner-document-create',
                    ...
                    const tenancyPanel = supportsTenancyRelations
                        ...
                        : '';
                    const documentsPanel = '<div class="domus-panel domus-panel-half">' + documentsHeader + '<div class="domus-panel-body">' +
                        Domus.Documents.renderLatestList('partner', id, {
                            pageSize: 10,
                            containerId: `domus-partner-documents-${id}`,
                            emptyActionId: 'domus-partner-documents-empty-create',
                            onEmptyAction: () => {
        ...
        function bindDetailActions(id) {
            ...
            document.getElementById('domus-partner-document-create')?.addEventListener('click', () => {
                Domus.Documents.openLinkModal('partner', id, () => renderDetail(id));
            });
        }
    ...
    Domus.PartnerRelations = (function() {
        function renderSection(partners, options = {}) {
            ...
            const body = rows.length
                ...
                : Domus.UI.buildEmptyStateAction('', { actionId: emptyAddId });
